(function() {
  ...
  var POPUP_ORIGIN_RIBBON_CLICK = 'RIBBON_CLICK';
  ...
  var Delivery = function(hostUrl, guid) {
    ...
    var ribbonClicked = function(event) {
    	loadSpecificVersion(getVersionForDisplay(containerOption), POPUP_ORIGIN_RIBBON_CLICK);
    };

    var loadSpecificVersion = function(version, popupOrigin) {
        ...
        (function(resource) {
          new ResourceLoader(hostUrl, delivery.getPopupGuid() + "/" + versionGuid).gather(resource.name, resource.type, function(data) {
            ...
            if (numberLoaded == uniqueResources.length) {
              delivery.popupOrigin = popupOrigin;
